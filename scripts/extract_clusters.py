...
4. Exports human-readable cluster summaries to outputs/cluster_summary_*.txt
...
for window in sorted(rolling_corrs.keys()):
    output_file = os.path.join(OUTPUT_DIR, f'cluster_summary_{window}day.txt')
...
print(f"  • {OUTPUT_FILE}")
for window in sorted(rolling_corrs.keys()):
    print(f"  • {os.path.join(OUTPUT_DIR, f'cluster_summary_{window}day.txt')}")
